 ...
 */
export function AnalyticsCarousel({ slides }: { slides: { key: string; label: string; content: ReactNode }[] }) {
  const t = useT();
  ...
  const onKey = (e: React.KeyboardEvent<HTMLDivElement>) => {
    if (!embla) return;
    if (e.key === "ArrowLeft") { embla.scrollPrev(); pauseAfterInteract(); }
    if (e.key === "ArrowRight") { embla.scrollNext(); pauseAfterInteract(); }
  };
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          {slides.map((s, i) => (
            <button
              key={s.key}
              type="button"
              onClick={() => { embla?.scrollTo(i); pauseAfterInteract(); }}
              className={cn(
        ...
        <div className="flex items-center gap-1">
          <Button
            ...
            onClick={() => { embla?.scrollPrev(); pauseAfterInteract(); }}
            aria-label="Previous"
          >
          ...
          <Button
            ...
            onClick={() => { embla?.scrollNext(); pauseAfterInteract(); }}
            aria-label="Next"
          >
          ...
          <Button
            ...
            className="gap-1.5"
          >
            {carouselAutoSlide ? <Pause className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />}
