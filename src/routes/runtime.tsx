...
function healthTone(h: ComponentHealth): StatusTone {
  return h === "healthy" ? "healthy" : h === "degraded" ? "degraded" :
    h === "input_required" ? "input_required" : h === "blocked" ? "blocked" :
    h === "stale" ? "stale" : "unavailable";
}
function providerTone(s: ProviderState): StatusTone {
  return s === "active" ? "healthy" : s === "ready" ? "healthy" :
    s === "testing" ? "input_required" : s === "degraded" ? "degraded" :
    s === "archived" ? "archived" : "unavailable";
}
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        ...
        actions={
          <>
            <Button size="sm" variant="outline" data-control-id={controls.runtime.refresh} className="gap-1.5">
              <RefreshCw className="h-3.5 w-3.5" />{t("common.refresh")}
            </Button>
            <BackendRequiredDialog
              controlId={controls.runtime.selfTest}
              trigger={<Button size="sm">{t("runtime.self_test")}</Button>}
              title={t("runtime.self_test")} payloadPreview={{ intent: "runtime.self_test" }}
            />
          </>
        }
      />
                ...
                <Card key={c.id}>
                  ...
                  <CardContent className="space-y-1 text-sm">
                    ...
                    <div className="grid grid-cols-2 gap-x-2 text-xs text-muted-foreground">
                      <div>{t("runtime.version")}</div><div className="text-right">{c.version}</div>
                      <div>{t("runtime.last_restart")}</div><div className="text-right"><TimeAgo iso={c.lastRestartAt} /></div>
                      <div>{t("runtime.last_healthy")}</div><div className="text-right"><TimeAgo iso={c.lastHealthyAt} /></div>
                    </div>
                    <div className="flex flex-wrap gap-2 border-t border-border pt-2">
                      <BackendRequiredDialog
                        controlId={controls.runtime.componentRestart}
                        trigger={<Button size="sm" variant="outline">{t("runtime.restart")}</Button>}
                        title={t("runtime.restart")} payloadPreview={{ intent: "runtime.restart", id: c.id }}
                      />
                      <Button size="sm" variant="ghost" data-control-id={controls.runtime.logsOpen}>Logs</Button>
                    </div>
                  </CardContent>
                </Card>
              ))}
            ...
            <Card><CardContent className="overflow-x-auto p-0">
              <table className="w-full text-sm">
                <thead className="border-b border-border text-left text-xs uppercase text-muted-foreground">
                  <tr>
                    <th className="px-4 py-2">Component</th><th className="px-2 py-2">Kind</th>
                    <th className="px-2 py-2">{t("col.status")}</th><th className="px-2 py-2">Version</th>
                    <th className="px-2 py-2">{t("runtime.last_healthy")}</th><th />
                  </tr>
                </thead>
                <tbody>
                  {(runtime.data ?? []).map((c) => (
                    <tr key={c.id} className="border-b border-border/60 last:border-none">
                      ...
                      <td className="px-2 py-2"><StatusBadge tone={healthTone(c.health)} /></td>
                      <td className="px-2 py-2 tabular-nums">{c.version}</td>
                      <td className="px-2 py-2"><TimeAgo iso={c.lastHealthyAt} /></td>
                      <td className="pr-3">
                        <BackendRequiredDialog
                          controlId={controls.runtime.componentRestart}
                          trigger={<Button size="sm" variant="ghost">{t("runtime.restart")}</Button>}
                          title={t("runtime.restart")} payloadPreview={{ intent: "runtime.restart", id: c.id }}
                        />
                      </td>
                    </tr>
        ...
        <TabsContent value="providers" className="mt-4">
          ...
          <div className="mb-3">
            <BackendRequiredDialog
              ...
              trigger={<Button size="sm" className="gap-1.5"><Plus className="h-3.5 w-3.5" />{t("provider.add")}</Button>}
              title={t("provider.add")}
              description={t("provider.note_inactive_default")}
              payloadPreview={{ intent: "provider.add", state: "inactive" }}
            />
          </div>
          {providers.isPending ? <LoadingState /> : (providers.data ?? []).length === 0 ? <EmptyState /> : (
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {(providers.data ?? []).map((p) => (
                <Card key={p.id}>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    <div className="grid grid-cols-2 gap-x-2 text-xs text-muted-foreground">
                      <div>Kind</div><div className="text-right">{p.kind}</div>
                      <div>Endpoint</div><div className="text-right"><Sensitive value={p.endpointMasked} /></div>
                      <div>Last tested</div><div className="text-right"><TimeAgo iso={p.lastTestedAt} /></div>
                    </div>
                    <div className="flex flex-wrap gap-1.5 border-t border-border pt-2">
                      <BackendRequiredDialog
                        controlId={controls.providers.test}
                        trigger={<Button size="sm" variant="outline">{t("provider.test")}</Button>}
                        title={t("provider.test")} payloadPreview={{ intent: "provider.test", id: p.id }}
                      />
                      <BackendRequiredDialog
                        controlId={controls.providers.edit}
                        trigger={<Button size="sm" variant="ghost">{t("common.edit")}</Button>}
                        title={t("common.edit")} payloadPreview={{ intent: "provider.edit", id: p.id }}
                      />
                      <BackendRequiredDialog
                        controlId={controls.providers.switchPreview}
                        trigger={<Button size="sm" variant="ghost">{t("provider.switch_preview")}</Button>}
                        title={t("provider.switch_preview")} payloadPreview={{ intent: "provider.switch_preview", id: p.id }}
                      />
                      {p.state === "active" ? (
                        <BackendRequiredDialog
                          controlId={controls.providers.deactivate}
                          trigger={<Button size="sm" variant="outline">{t("provider.deactivate")}</Button>}
                          title={t("provider.deactivate")} payloadPreview={{ intent: "provider.deactivate", id: p.id }}
                        />
                      ) : p.state === "archived" ? (
                        <>
                          <BackendRequiredDialog
                            ...
                            title={t("common.restore")} payloadPreview={{ intent: "provider.restore", id: p.id }}
                          />
                          <BackendRequiredDialog
                            controlId={controls.providers.deletePermanent}
                            trigger={<Button size="sm" variant="destructive">{t("common.delete_permanent")}</Button>}
                            title={t("common.delete_permanent")} payloadPreview={{ intent: "provider.delete_permanent", id: p.id }}
                          />
                        </>
                      ) : (
                        <BackendRequiredDialog
                          ...
                          title={t("provider.activate")} payloadPreview={{ intent: "provider.activate", id: p.id, warn: "Provider B never auto-activates" }}
                        />
                      )}
                      <BackendRequiredDialog
                        controlId={controls.providers.archive}
                        trigger={<Button size="sm" variant="ghost">{t("common.archive")}</Button>}
                        title={t("common.archive")} payloadPreview={{ intent: "provider.archive", id: p.id }}
                      />
                    </div>
                  </CardContent>
                </Card>
        ...
        <TabsContent value="versions" className="mt-4">
          <div className="flex flex-wrap gap-2">
            <BackendRequiredDialog
              ...
              title={t("runtime.update")} payloadPreview={{ intent: "runtime.update" }}
            />
            <BackendRequiredDialog
              controlId={controls.runtime.rollbackOpen}
              trigger={<Button size="sm" variant="outline">{t("runtime.rollback")}</Button>}
              title={t("runtime.rollback")} payloadPreview={{ intent: "runtime.rollback" }}
            />
          </div>
          <div className="mt-3">
            <EmptyState />
