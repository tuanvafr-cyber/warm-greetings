...
export const Route = createFileRoute("/sources")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Manage signal sources: Active, Performance, Archive." },
    ],
  }),
...
function tone(l: SourceLifecycle): StatusTone {
  return l === "enabled" ? "active"
    : l === "disabled" ? "disabled"
    : l === "draining" ? "draining"
    : l === "degraded" ? "degraded" : "archived";
}
...
function SourcesPage() {
  ...
  const filter = (list: Source[]) =>
    list.filter((s) => !search || s.displayName.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="flex flex-col gap-4">
        ...
        actions={
          <>
            <BackendRequiredDialog
              ...
              trigger={<Button size="sm" variant="outline" className="gap-1.5"><Upload className="h-3.5 w-3.5" />{t("sources.import")}</Button>}
            ...
            <BackendRequiredDialog
              ...
              trigger={<Button size="sm" className="gap-1.5"><Plus className="h-3.5 w-3.5" />{t("sources.add")}</Button>}
              title={t("sources.add")}
              description={t("backend.desc")}
      ...
      <Tabs defaultValue="active">
        ...
        <TabsContent value="active" className="mt-4">
          ...
          {q.isPending ? <LoadingState /> : active.length === 0 ? <EmptyState /> : (
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {active.map((s) => <ActiveCard key={s.id} s={s} />)}
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="archive" className="mt-4">
          ...
          {q.isPending ? <LoadingState /> : archive.length === 0 ? <EmptyState /> : (
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {archive.map((s) => <ArchiveCard key={s.id} s={s} />)}
            </div>
          )}
        </TabsContent>
      </Tabs>
    ...
    <Card>
      ...
      <CardContent className="space-y-2 text-sm">
        <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-xs text-muted-foreground">
          <div>{t("sources.parser")}</div><div className="text-right">{s.parserProfile}@{s.parserVersion}</div>
          <div>{t("sources.symbols")}</div><div className="text-right">{s.symbolProfile}</div>
          <div>Realtime · History</div>
          <div className="text-right">{s.realtime ? "✓" : "—"} · {s.history ? "✓" : "—"}</div>
          <div>{t("sources.last_signal")}</div><div className="text-right"><TimeAgo iso={s.lastSignalAt} /></div>
          <div>Open lifecycle</div><div className="text-right tabular-nums">{s.openLifecycleCount}</div>
        </div>
        <div className="flex flex-wrap items-center gap-2 border-t border-border pt-2">
          <div className="flex items-center gap-2">
            <Switch
              checked={s.lifecycle === "enabled"}
              data-control-id={controls.sources.toggle}
            />
          ...
          <div className="ml-auto flex flex-wrap gap-1.5">
            <BackendRequiredDialog
              ...
              trigger={<Button size="sm" variant="outline">{t("common.verify")}</Button>}
              title={t("common.verify")} payloadPreview={{ intent: "source.verify", id: s.id }}
            />
            <BackendRequiredDialog
              controlId={controls.sources.edit}
              trigger={<Button size="sm" variant="ghost">{t("common.edit")}</Button>}
              title={t("common.edit")} payloadPreview={{ intent: "source.edit", id: s.id }}
            />
            <BackendRequiredDialog
              controlId={controls.sources.archive}
              trigger={<Button size="sm" variant="ghost">{t("common.archive")}</Button>}
              title={t("common.archive")} payloadPreview={{ intent: "source.archive", id: s.id }}
            />
          </div>
        </div>
      </CardContent>
      ...
      <CardContent className="space-y-2 text-sm">
        ...
        <div className="flex flex-wrap gap-1.5 border-t border-border pt-2">
          <BackendRequiredDialog
            ...
            title={t("common.restore")} payloadPreview={{ intent: "source.restore", id: s.id }}
          />
          <Button size="sm" variant="ghost" data-control-id={controls.sources.export} className="gap-1">
          ...
          <BackendRequiredDialog
            ...
            title={t("common.delete_permanent")} payloadPreview={{ intent: "source.delete_permanent", id: s.id }}
          />
        </div>
      </CardContent>
    </Card>
          ...
          <tbody>
            {sources.map((s) => (
              <tr key={s.id} className="border-b border-border/60 last:border-none">
                ...
                <td className="px-2 py-2 text-right"><Pct value={s.executionRate} /></td>
                <td className="px-2 py-2 text-right"><Pct value={s.technicalFailureRate} /></td>
                <td className="px-2 py-2 text-right tabular-nums">{s.avgLatencyMs}ms</td>
                <td className="px-2 py-2">{s.bestSymbol} · {String(s.bestHour).padStart(2, "0")}h</td>
