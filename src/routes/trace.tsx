...
export const Route = createFileRoute("/trace")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Immutable lifecycle trace across the SignalOps pipeline." },
    ],
  }),
  component: TracePage,
});
...
function stepTone(s: TraceStep): StatusTone {
  return s.outcome === "ok" ? "healthy" : s.outcome === "blocked" ? "blocked"
    : s.outcome === "error" ? "degraded" : "input_required";
}
...
function TracePage() {
  ...
  const filtered = useMemo(() => (q.data ?? []).filter((tr) =>
    !txt || tr.correlationId.includes(txt) || tr.signalId?.includes(txt) || tr.orderId?.includes(txt)
  ), [q.data, txt]);
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        ...
        actions={
          <>
            <Button size="sm" variant="outline" data-control-id={controls.trace.exportJson} className="gap-1.5">
              <Download className="h-3.5 w-3.5" />JSON
            </Button>
            <Button size="sm" variant="outline" data-control-id={controls.trace.exportCsv} className="gap-1.5">
              ...
              {filtered.map((tr) => (
                <li key={tr.correlationId}>
                  ...
                  >
                    ...
                    <div className="text-[10px] text-muted-foreground"><TimeAgo iso={tr.createdAt} /></div>
                  </button>
                </li>
              ))}
            </ul>
      ...
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center justify-between text-sm">
          ...
          <div className="flex gap-1">
            <Button size="icon" variant="ghost" data-control-id={controls.trace.copyId}
              onClick={() => navigator.clipboard?.writeText(tr.correlationId)}>
              <Copy className="h-3.5 w-3.5" />
            </Button>
            {tr.signalId ? <Button asChild size="sm" variant="ghost" data-control-id={controls.trace.openSignal}>
              <Link to="/signals">Signal</Link>
            </Button> : null}
            {tr.orderId ? <Button asChild size="sm" variant="ghost" data-control-id={controls.trace.openOrder}>
          ...
          {tr.steps.map((s, i) => (
            <li key={i} className="relative">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="ml-auto text-xs text-muted-foreground"><TimeAgo iso={s.at} /></span>
              </div>
              <p className="mt-0.5 text-sm">{s.detail}</p>
              {Object.keys(s.payloadRedacted).length ? (
                <details className="mt-1">
                  <summary className="cursor-pointer text-xs text-muted-foreground">{t("trace.payload_redacted")}</summary>
