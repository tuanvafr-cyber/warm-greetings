...
const STATUS_OPTIONS: (SignalStatus | "all")[] =
  ["all", "parsed", "deduped", "risk_blocked", "policy_blocked", "executed", "partial", "technical_failed"];

function statusTone(s: SignalStatus): StatusTone {
  switch (s) {
    case "executed": return "healthy";
    case "partial": return "input_required";
    case "risk_blocked":
    case "policy_blocked": return "blocked";
    case "technical_failed": return "degraded";
    case "deduped": return "stale";
    default: return "input_required";
  }
}
    ...
    <div className="flex flex-col gap-4">
        ...
        <Card>
          <CardContent className="overflow-x-auto p-0">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((s) => (
                  <tr key={s.id} className="cursor-pointer border-b border-border/60 last:border-none hover:bg-muted/50"
                      onClick={() => setOpenId(s.id)} data-control-id={controls.signals.openDetail}>
                    <td className="px-4 py-2 text-xs text-muted-foreground"><TimeAgo iso={s.receivedAt} /></td>
                    ...
                    <td className="px-2 py-2"><StatusBadge tone={statusTone(s.status)} /></td>
                    <td className="px-4 py-2 font-mono text-xs text-muted-foreground">{s.correlationId}</td>
                    <td className="pr-3 text-right"><ExternalLink className="inline h-3.5 w-3.5 text-muted-foreground" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Sheet open={!!opened} onOpenChange={(v) => !v && setOpenId(null)}>
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{opened?.symbol} · {opened?.side.toUpperCase()}</SheetTitle>
          </SheetHeader>
  ...
  return (
    <div className="mt-4 space-y-4 text-sm">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground"><TimeAgo iso={s.receivedAt} /></span>
      </div>
      <Detail label={t("col.source")} value={s.sourceName} />
      <Detail label={t("signals.correlation")} value={
        <span className="inline-flex items-center gap-1 font-mono text-xs">
          {s.correlationId}
          <Button size="icon" variant="ghost" data-control-id={controls.signals.copyCorrelation}
      ...
      <div>
        <div className="mb-1 text-xs font-medium uppercase text-muted-foreground">{t("signals.parsed_fields")}</div>
        <div className="grid grid-cols-3 gap-2 rounded-md border border-border p-2 text-sm">
          <div>Entry: <span className="tabular-nums">{s.parsedEntry ?? "—"}</span></div>
          <div>SL: <span className="tabular-nums">{s.parsedSl ?? "—"}</span></div>
          <div>TP: <span className="tabular-nums">{s.parsedTp ?? "—"}</span></div>
        </div>
        <div className="mt-1 text-xs text-muted-foreground">Parser: {s.parserProfile}@{s.parserVersion}</div>
      </div>
      <div>
        <div className="mb-1 text-xs font-medium uppercase text-muted-foreground">{t("signals.original")}</div>
        <pre className="max-h-40 overflow-auto whitespace-pre-wrap rounded-md border border-border bg-muted/40 p-2 text-xs">{s.originalText}</pre>
      </div>
      {s.reason ? (
        <div>
          <div className="mb-1 text-xs font-medium uppercase text-muted-foreground">{t("signals.reason")}</div>
      ...
      <div>
        <div className="mb-1 text-xs font-medium uppercase text-muted-foreground">{t("signals.destination")}</div>
        <div className="flex flex-wrap gap-1.5">
          {s.destinationAccountIds.map((id) => (
            <span key={id} className="rounded-md bg-muted px-2 py-0.5 text-xs">{id}</span>
          ))}
        </div>
      </div>
      <div>
        <div className="mb-1 text-xs font-medium uppercase text-muted-foreground">{t("signals.related_orders")}</div>
        {s.relatedOrderIds.length ? (
          <div className="flex flex-wrap gap-1.5">
            {s.relatedOrderIds.map((id) => (
              <Link key={id} to="/orders" data-control-id={controls.signals.openOrders}
                className="rounded-md border border-border px-2 py-0.5 text-xs hover:bg-muted">{id}</Link>
            ))}
          </div>
      ...
      <div className="flex gap-2 border-t border-border pt-3">
        ...
        <Button size="sm" variant="ghost" data-control-id={controls.signals.inspectParser}>Inspect parser</Button>
      </div>
    </div>
  );
}
