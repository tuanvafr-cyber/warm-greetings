...
function PositionsPage() {
  ...
  const filtered = useMemo(() => (q.data ?? []).filter((p) =>
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        ...
        actions={
          <>
            <Button size="sm" variant="outline" data-control-id={controls.positions.refresh} className="gap-1.5">
              <RefreshCw className="h-3.5 w-3.5" />{t("common.refresh")}
            </Button>
            <Button size="sm" variant="outline" data-control-id={controls.positions.export} className="gap-1.5">
      ...
      <FilterBar>
        ...
        <div className="flex gap-1" data-control-id={controls.positions.filterSide}>
          {(["all", "buy", "sell"] as const).map((v) => (
            <Button key={v} size="sm" variant={side === v ? "default" : "ghost"}
              className="h-7 px-2 text-xs uppercase" onClick={() => setSide(v)}>{v}</Button>
          ))}
        </div>
      </FilterBar>
      ...
      <section>
        ...
        {q.isPending ? <LoadingState /> : open.length === 0 ? <EmptyState /> : <PosTable rows={open} />}
      </section>

      <section>
        <h2 className="mb-2 text-sm font-semibold">{t("positions.pending")}</h2>
        {q.isPending ? <LoadingState /> : pending.length === 0 ? <EmptyState /> : <PosTable rows={pending} />}
      </section>
    </div>
  );
}
  ...
  return (
    <Card><CardContent className="overflow-x-auto p-0">
        ...
        <tbody>
          {rows.map((p) => (
            <tr key={p.id} className="border-b border-border/60 last:border-none">
              ...
              <td className="px-2 py-2 text-right tabular-nums">{p.sl?.toFixed(2)} / {p.tp?.toFixed(2)}</td>
              <td className="px-2 py-2 text-right"><NativeAmount amount={p.nativeVolume} currency={p.accountLabel.includes("USC") ? "USC" : "USD"} /></td>
              <td className="px-2 py-2 text-right tabular-nums">{p.pips}</td>
              <td className="px-2 py-2 text-right"><MoneyUsd value={p.floatingPnlUsd} colorize /></td>
              <td className="px-2 py-2"><Sensitive value={p.ticket} /></td>
              <td className="px-4 py-2 font-mono text-xs text-muted-foreground">{p.correlationId}</td>
              <td className="pr-3">
                <div className="flex gap-1">
                  <Button asChild size="sm" variant="ghost" data-control-id={controls.positions.openSignal}>
                    <Link to="/signals">S</Link>
                  </Button>
                  <Button asChild size="sm" variant="ghost" data-control-id={controls.positions.openTrace}>
      ...
      <div className="border-t border-border p-2 text-right">
        ...
        <span className="ml-2 text-xs text-muted-foreground">Close-actions require SignalOps backend.</span>
      </div>
    </CardContent></Card>
  );
}
