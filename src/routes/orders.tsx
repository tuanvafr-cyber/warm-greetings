...
export const Route = createFileRoute("/orders")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Bounded order history with filters and CSV/JSON export." },
    ],
  }),
...
function resultTone(r: OrderResult): StatusTone {
  return r === "win" ? "healthy" : r === "loss" ? "blocked" : r === "pending" ? "input_required" : "stale";
}
      ...
      {q.isPending ? <LoadingState /> : filtered.length === 0 ? <EmptyState /> : (
        <Card><CardContent className="overflow-x-auto p-0">
          <table className="w-full text-sm">
            ...
            <tbody>
              {paged.map((o: Order) => (
                <tr key={o.id} className="border-b border-border/60 last:border-none hover:bg-muted/40">
                  <td className="px-4 py-2 text-xs text-muted-foreground">{o.closedAt.slice(0, 16).replace("T", " ")}</td>
                  ...
                  <td className="px-2 py-2 text-right"><MoneyUsd value={o.netPnlUsd} colorize /></td>
                  <td className="px-2 py-2"><Sensitive value={o.ticket} /></td>
                  <td className="px-2 py-2"><StatusBadge tone={resultTone(o.result)} /></td>
                  <td className="pr-3">
                    <Button asChild size="sm" variant="ghost" data-control-id={controls.orders.openTrace}>
