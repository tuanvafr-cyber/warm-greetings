...
export const Route = createFileRoute("/risk")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Effective risk policy, drafts, previews and version history." },
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-1">
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <Row label={t("risk.native_amount")} value={
                <NativeAmount amount={effective?.nativeRiskAmount ?? 0} currency={effective?.nativeCurrency ?? "USC"} />
              } />
              <Row label={t("risk.daily_loss")} value={<MoneyUsd value={effective?.dailyLossLimitUsd ?? 0} />} />
              <Row label={t("risk.drawdown")} value={<MoneyUsd value={effective?.drawdownLimitUsd ?? 0} />} />
              <Row label={t("risk.margin_buffer")} value={`${effective?.marginBufferPct}%`} />
              <Row label={t("risk.budget")} value={<MoneyUsd value={effective?.riskBudgetUsd ?? 0} />} />
            ...
            <CardContent className="space-y-3">
              <div>
                ...
                <select className="mt-1 h-9 w-full rounded-md border border-input bg-background px-2 text-sm"
                  data-control-id={controls.risk.accountScope} value={scope} onChange={(e) => setScope(e.target.value)}>
              ...
              <div className="flex flex-wrap gap-2 border-t border-border pt-3">
                <BackendRequiredDialog
                  ...
                  trigger={<Button size="sm" variant="outline">{t("risk.preview_impact")}</Button>}
                  title={t("risk.preview_impact")} payloadPreview={{ intent: "risk.preview", scope, draft }}
                />
                <BackendRequiredDialog
                  controlId={controls.risk.compareVersions}
                  trigger={<Button size="sm" variant="ghost">{t("risk.compare_versions")}</Button>}
                  title={t("risk.compare_versions")} payloadPreview={{ intent: "risk.compare", from: effective?.version, to: (effective?.version ?? 0) - 1 }}
                />
                <BackendRequiredDialog
                ...
                <tbody>
                  {(q.data ?? []).map((v) => (
                    <tr key={v.version} className="border-b border-border/60 last:border-none">
                      ...
                      <td className="px-2 py-2"><NativeAmount amount={v.nativeRiskAmount} currency={v.nativeCurrency} /></td>
                      <td className="px-2 py-2 text-right"><MoneyUsd value={v.dailyLossLimitUsd} /></td>
                      <td className="px-2 py-2 text-right"><MoneyUsd value={v.drawdownLimitUsd} /></td>
                      ...
                      <td className="pr-3">
                        <BackendRequiredDialog
                          ...
                          trigger={<Button size="sm" variant="ghost">{t("risk.restore")}</Button>}
                          title={t("risk.restore")} payloadPreview={{ intent: "risk.restore", version: v.version }}
                        />
                      </td>
                    </tr>
                  ))}
